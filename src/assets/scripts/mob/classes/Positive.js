...
export default class Positive extends Phaser.Physics.Arcade.Group {
    ...
    createFirstElements() {
        let data_1 = DATA[0]
        let elem_1 = new PositiveObject(this.scene, X_3, 400, 'positive_spritesheet', 'positive1', 20 )
        elem_1.move()
        this.add(elem_1)

        let data_2 = DATA[1]
        let elem_2 = new PositiveObject(this.scene, X_1, -200, 'positive_spritesheet', 'positive2', -20)
        elem_2.move()
        this.add(elem_2)

        let data_3 = DATA[2]
        let elem_3 = new PositiveObject(this.scene, X_1, -760, 'positive_spritesheet', 'positive3', -20)
        elem_3.move()
        this.add(elem_3)

        let data_4 = DATA[3]
        let elem_4 = new PositiveObject(this.scene, X_2, -1290, 'positive_spritesheet', 'positive4', 20)
        elem_4.move()
        this.add(elem_4)

        let data_5 = DATA[4]
        let elem_5 = new PositiveObject(this.scene, X_2, -1640, 'positive_spritesheet', 'positive5', -20)
        elem_5.move()
        this.add(elem_5)        

        let data_6 = DATA[5]
        let elem_6 = new PositiveObject(this.scene, X_2, -1860, 'positive_spritesheet', 'positive6', 20)
        elem_6.move()
        this.add(elem_6)
